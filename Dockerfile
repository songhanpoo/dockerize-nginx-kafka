...
RUN GPG_KEYS=B0F4253373F8F6F510D42178520A9993A1C052F8 \
	&& CONFIG="\
		...
		--add-dynamic-module=/usr/src/echo-nginx-module-$NGINX_ECHO_MODULE_VERSION \
		...
		--add-dynamic-module=/usr/src/nginx-kafka-log-module-$NGINX_KAFKA_LOG_MODULE_VERSION \
		--with-threads \
		--with-stream \
	...
	&& tar -zxC /usr/src -f ngx_echo_module-$NGINX_ECHO_MODULE_VERSION.tar.gz \
	&& tar -zxC /usr/src -f ngx_kafka_log_module-$NGINX_KAFKA_LOG_MODULE_VERSION.tar.gz \
	...
	&& rm ngx_echo_module-$NGINX_ECHO_MODULE_VERSION.tar.gz \
	&& rm ngx_kafka_log_module-$NGINX_KAFKA_LOG_MODULE_VERSION.tar.gz \
	...
	&& mv objs/ngx_http_kafka_log_module.so objs/ngx_http_kafka_log_module-debug.so \
	...
	&& install -m755 objs/ngx_http_kafka_log_module-debug.so /usr/lib/nginx/modules/ngx_http_kafka_log_module-debug.so \
	...
	&& apk add --no-cache tzdata --virtual .nginx-rundeps $runDeps \
	&& apk del .build-deps \
	&& apk del .gettext \
...
RUN apk update && apk add libsasl
